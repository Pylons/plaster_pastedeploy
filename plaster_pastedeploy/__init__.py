...
class Loader(object):
    def __init__(self, uri):
        """This is a loader conforming to the required interface defined by :class:`plaster.interfaces.Loader`.
        Given a uri to a configuration source, this can load and return wsgi applications, servers, filters and
        settings from .ini files written against the PasteDeploy spec.

        :param uri: A specification for the configuration source formatted as "{scheme}:{file path}#{named section}.
        the scheme and named section portions are optional and will get defaults during loading.
        """
    ...
    def get_settings(self, name=None, defaults=None):
        """Gets a named section from the configuration source. ``name`` must either be included in the ``uri`` passed
        to the class during instantiation, or must be passed to get settings.

        :param name: a :class:`str` representing the section you want to retrieve from the configuration source.
        :param defaults: a :class:`dict` that will get passed to :class:`configparser.ConfigParser` and will populate
        the DEFAULT section.
        :return: A :class:`collections.OrderedDict` with key value pairs as parsed by :class:`configparser.ConfigParser`.
        """
    ...
    def get_wsgi_app(self, name=None, defaults=None, relative_to=None):
        ...
        per the PasteDeploy configuration format and loading mechanism.

        :param name: The named wsgi app to find, load and return. Defaults to ``None`` which becomes ``main`` inside
        :func:`paste.deploy.loadapp`.
        :param defaults: The ``global_conf`` that will be used during app instantiation.
        :param relative_to: A path indicating what path the uri configuration file is relative to. Defaults to ``None``.
        and may be be set internally to :func:`os.getcwd` if ``self.uri`` is not an absolute path.
    ...
    def get_wsgi_server(self, name=None, defaults=None, relative_to=None):
        """Reads the configuration source and finds and loads a wsgi server defined by the server entry with the name
        ``name`` per the PasteDeploy configuration format and loading mechanism.

        :param name: The named wsgi server to find, load and return. Defaults to ``None`` which becomes ``main`` inside
        :func:`paste.deploy.loadserver`.
        :param defaults: The ``global_conf`` that will be used during server instantiation.
        :param relative_to: A path indicating what path the uri configuration file is relative to. Defaults to ``None``.
        and may be be set internally to :func:`os.getcwd` if ``self.uri`` is not an absolute path.
    ...
    def get_wsgi_filter(self, name=None, defaults=None, relative_to=None):
        """Reads the configuration soruce and finds and loads a wsgi filter defined by the filter entry with the name
        ``name`` per the PasteDeploy configuration format and loading mechanism.

        :param name: The named wsgi filter to find, load and return. Defaults to ``None`` which becomes ``main`` inside
        :func:`paste.deploy.loadfilter`.
        :param defaults: The ``global_conf`` that will be used during filter instantiation.
        :param relative_to: A path indicating what path the uri configuration file is relative to. Defaults to ``None``.
        and may be be set internally to :func:`os.getcwd` if ``self.uri`` is not an absolute path.
        :return:
        """
    ...
    def get_wsgi_app_config(self, name=None, defaults=None, relative_to=None):
        ...
        :param name: The named wsgi app for which to find the settings. Defaults to ``None`` which becomes ``main``
        inside :func:`paste.deploy.loadapp`.
        :param defaults: The ``global_conf`` that will be used during settings generation.
        :param relative_to: A path indicating what path the uri configuration file is relative to. Defaults to ``None``.
        and may be be set internally to :func:`os.getcwd` if ``self.uri`` is not an absolute path.
...
class ConfigDict(OrderedDict, loadwsgi.AttrDict):
    """A subclass to use so that the return values for getting settings can be an instance of
    :class:`collections.OrderedDict` and :class:`loadwsgi.AttrDict`.

    This is so that any code writen against PasteDeploy will get an expected return value.
    """
    pass


def _ordered_dict_from_attr_dict(attr_dict):
    """Given an attr_dict turns it into a ConfigDict. This is an :class:`collections.OrderedDict` with the following
